                ...
                <table className="mt-4 min-w-full leading-normal" {...getTableProps()}>
                    ...
                    {headerGroups.map((headerGroup) => (
                      <tr {...headerGroup.getHeaderGroupProps()}>
                        {headerGroup.headers.map((column) => (
                          <th className="py-3 border-b-2 border-gray-200 font-normal text-left text-xs lg:text-sm text-gray-400 w-20 tracking-wider" {...column.getHeaderProps()}>{column.render('Header')}</th>
                        ))}
                      </tr>
                    ))}
                  </thead>
                  <tbody {...getTableBodyProps()}>
                    {page.map((row, i) => {
                      ...
                      return (
                        <tr {...row.getRowProps()}>
                          {row.cells.map((cell) => <td className="table-font py-1 border-b-2 border-gray-200 text-left text-xs lg:text-sm text-black w-20 tracking-wider" {...cell.getCellProps()}>{cell.render('Cell')}</td>)}
                        </tr>
                      );
                    })}
                  </tbody>
